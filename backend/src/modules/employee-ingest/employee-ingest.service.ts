...
import { Role, SkillChangeType, SkillRatingStatus, TaskStatus } from "@prisma/client";
...
import {
  ...
  normalizeSkillHeader,
  normalizeSkillCell,
  normalizeTaskCell,
  validateEmployeeHeader,
} from "./employee-ingest.validators";
import { EmployeeIngestError, EmployeeIngestRow, EmployeeIngestSummary } from "./employee-ingest.types";
...
const buildEmail = (base: string, counter: number) => `${base}employee${counter}@gmail.com`;

const toLower = (value: string) => value.toLowerCase();

export const ingestEmployees = async (
  buffer: Buffer
): Promise<{ summary: EmployeeIngestSummary; fatal: boolean }> => {
  ...
  const headerResult = validateEmployeeHeader(rows[0]);
  if (!headerResult.ok) {
    return {
      fatal: true,
      summary: {
  ...
  const errors: EmployeeIngestError[] = [];
  const validRows: EmployeeIngestRow[] = [];

  const headerLength = rows[0].length;
  const skillHeaders = rows[0]
    .slice(3, headerLength - 4)
    .map((value) => normalizeSkillHeader(value))
  ...
  dataRows.forEach((row, index) => {
    ...
    const department = normalizeDepartmentName(row[2]);

    const reportsTo = normalizeEmail(row[headerLength - 4]);
    const project = normalizeProjectName(row[headerLength - 3]);
    const deliverable = normalizeDeliverableName(row[headerLength - 2]);
    const tasksCell = normalizeTaskCell(row[headerLength - 1]);

    const skills = skillHeaders
      .map((skillName, index) => {
        const raw = normalizeSkillCell(row[3 + index]);
    ...
    const invalidSkill = skills.find(
      (entry) => !Number.isFinite(entry.rating) || entry.rating < 1 || entry.rating > 5
    );
    if (invalidSkill) {
      errors.push({
        row: rowNumber,
  ...
  const managerMap = new Map(
    managers.map((user) => [toLower(user.email), { profileId: user.profile?.id, userId: user.id }])
  );
  ...
  let tasksCreated = 0;
  const createdEmployees: EmployeeIngestSummary["employees"] = [];

  await prisma.$transaction(async (tx) => {
    const projectMap = new Map<string, { id: string; name: string }>();
    const deliverableMap = new Map<string, { id: string; projectId: string }>();
    ...
    for (const row of validRows) {
      ...
      const base = normalizeEmailBase(row.name);
      let counter = 1;
  ...
  const summary: EmployeeIngestSummary = {
    processed: dataRows.length,
    projectsCreated,
    deliverablesCreated,
    employeesCreated,
