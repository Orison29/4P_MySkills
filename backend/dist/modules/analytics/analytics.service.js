...
exports.getDashboardStats = exports.getSkillProgressTimeline = exports.getSkillSpectrumByDepartment = exports.getAllEmployeesOverview = exports.getEmployeeSkillProgress = exports.getLearningSpeed = void 0;
const db_1 = require("../../utils/db");
const client_1 = require("@prisma/client");
function categorizeLearningSpeed(score) {
    if (score >= 0.03)
        return "FAST";
    if (score >= 0.01)
        return "MODERATE";
...
const getLearningSpeed = async (startDate, endDate) => {
    const employees = await db_1.prisma.employeeProfile.findMany({
        where: { user: { role: client_1.Role.EMPLOYEE } },
        select: {
            id: true,
            fullname: true,
    ...
    const dateFilter = startDate || endDate
        ...
        : {};
    const logs = await db_1.prisma.skillProgressLog.findMany({
    ...
    for (const log of logs) {
        if (!byEmployee.has(log.employeeId))
            byEmployee.set(log.employeeId, new Map());
        const bySkill = byEmployee.get(log.employeeId);
        if (!bySkill.has(log.skillId))
            bySkill.set(log.skillId, []);
    ...
    const employeeResults = employees.map((emp) => {
        ...
        if (bySkill) {
            for (const skillLogs of bySkill.values()) {
                if (skillLogs.length < 2)
                    continue;
                const first = skillLogs[0];
                const last = skillLogs[skillLogs.length - 1];
                const delta = last.newRating - first.newRating;
                const days = Math.max(1, (last.changedAt.getTime() - first.changedAt.getTime()) / (1000 * 60 * 60 * 24));
                velocities.push(delta / days);
            }
        }
        const score = velocities.length > 0
    ...
    for (const emp of employeeResults) {
        if (!deptMap.has(emp.department))
            deptMap.set(emp.department, { scores: [], count: 0 });
...
const getEmployeeSkillProgress = async (employeeId) => {
    ...
    const employeeSkills = await db_1.prisma.employeeSkill.findMany({
        where: { employeeId },
        include: {
            skill: true
        }
    });
    const progressLogs = await db_1.prisma.skillProgressLog.findMany({
        where: { employeeId },
        include: {
            skill: true,
            reviewer: {
...
const getAllEmployeesOverview = async () => {
    const employees = await db_1.prisma.employeeProfile.findMany({
        include: {
            department: {
                select: {
    ...
    return employees.map((emp) => {
        ...
        const approvedSkills = emp.employeeSkills.filter((es) => es.status === "APPROVED" || es.status === "EDITED").length;
        const pendingSkills = emp.employeeSkills.filter((es) => es.status === "PENDING").length;
        // Calculate average rating (approved ratings only)
        const approvedRatings = emp.employeeSkills
            .filter((es) => es.approvedRating !== null)
            .map((es) => es.approvedRating);
        const averageRating = approvedRatings.length > 0
            ? approvedRatings.reduce((sum, rating) => sum + rating, 0) /
                approvedRatings.length
            : 0;
        // Get last updated date
        const lastUpdated = emp.employeeSkills.length > 0
            ? emp.employeeSkills.reduce((latest, es) => es.updatedAt > latest ? es.updatedAt : latest, emp.employeeSkills[0].updatedAt)
            : null;
        return {
            id: emp.id,
            fullname: emp.fullname,
...
const getSkillSpectrumByDepartment = async (skillId) => {
...
const getSkillProgressTimeline = async (employeeId, skillId) => {
    const employee = await db_1.prisma.employeeProfile.findUnique({
        where: { id: employeeId },
        select: {
            fullname: true
    ...
    const timeline = await db_1.prisma.skillProgressLog.findMany({
        where: {
            employeeId,
            skillId
        },
    ...
    const currentSkill = await db_1.prisma.employeeSkill.findUnique({
        where: {
            employeeId_skillId: {
                employeeId,
                skillId
...
const getDashboardStats = async () => {
    const activeProjects = await db_1.prisma.project.count({
        where: { status: "ACTIVE" }
    });
    const totalEmployees = await db_1.prisma.employeeProfile.count();
    const pendingAssignments = await db_1.prisma.assignmentRequest.count({
        where: { status: "PENDING" }
    });
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    const newSkills = await db_1.prisma.skill.count({
        where: { createdAt: { gte: thirtyDaysAgo } }
    });
    // Get skill progress logs for the last 30 days
    const recentLogs = await db_1.prisma.skillProgressLog.findMany({
