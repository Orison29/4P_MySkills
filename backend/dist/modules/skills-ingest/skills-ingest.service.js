...
exports.ingestSkills = void 0;
const sync_1 = require("csv-parse/sync");
const db_1 = require("../../utils/db");
const skills_ingest_validators_1 = require("./skills-ingest.validators");
...
const ingestSkills = async (buffer, dupStrategy = "ignore") => {
    ...
    const headerResult = (0, skills_ingest_validators_1.validateSkillsHeader)(rows[0]);
    if (!headerResult.ok) {
        return {
            fatal: true,
            summary: {
    ...
    dataRows.forEach((row, index) => {
        ...
        const name = (0, skills_ingest_validators_1.normalizeSkillName)(rawValue);
        if (!name) {
    ...
    if (uniqueNames.length > 0) {
        const existing = await db_1.prisma.skill.findMany({
            where: {
    ...
    if (toCreate.length > 0) {
        const result = await db_1.prisma.skill.createMany({
...
exports.ingestSkills = ingestSkills;
